...
const ViewResume = () => {
  ...
  useEffect(() => {
    const fetchResume = async () => {
      ...
      try {
        const res = await api.get(`/api/resume/getResume/${currentUser._id}`);
  ...
  const handleDelete = async (e, resume) => {
    e.preventDefault();
    setIsLoading(true);
    const response = await api.delete(`/api/resume/deleteResume/${resume._id}`);
            ...
            {resumes.map((resume, index) => (
              <div key={index} className="m-4 float-left cursor-pointer">
                <div>
                  ...
                  <button
                    className="bg-red-500 p-2 m-6 rounded text-slate-100"
